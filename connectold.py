...
def printboard(game: 'GameState'): #prints out the Connect Four board
    ...
    for row in range(connectfour.BOARD_ROWS):
        for col in range(connectfour.BOARD_ROWS+1):
...
def main(): #main function
    ...
    while not done:                #the game part of the game
        try:
            ...
            if (movetype == 'Drop' or movetype == 'drop' and connectfour.winner(game) == 0): # Checks the move type and if there is a winner already or not
                column = command[5:]
                column = column.strip()
                column = int(column)
            ...
            elif (movetype == 'Pop' or movetype == 'pop' and connectfour.winner(game) == 0): #Checks the move type and if there is a winner already or not
                column = command[4:]
                column = column.strip()
                column = int(column)
